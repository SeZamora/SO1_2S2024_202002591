...
func enviarMensajeKafka(producer *kafka.Producer, topic string, mensaje []byte) error {
	...
	flushTimeout := 5000 // Tiempo de espera para flush en milisegundos (5 segundos)
	remainingMessages := producer.Flush(flushTimeout)
	if remainingMessages > 0 {
		log.Printf("Boxeo No se pudieron enviar todos los mensajes. Mensajes restantes: %d", remainingMessages)
		return fmt.Errorf("Boxeo No se pudieron enviar todos los mensajes. Mensajes restantes: %d", remainingMessages)
	}
...
func main() {
	...
	log.Printf("Servidor de Boxeo escuchando en el puerto 50054")
